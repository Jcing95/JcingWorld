...
public class Terrain {
	...
	public Terrain(float gridX, float gridZ, Loader loader, TerrainShader shader, TextureAtlas textureAtlas, BaseImage blendMap) {
		...
		this.gridX = gridX;
		this.gridZ = gridZ;
		borders = new Terrain[4]; // TODO: terrainborders: use Vecs and get
	...
	private void generateTiles() {
		...
		for (int i = 0; i < VERTEX_COUNT; i += 2) { // i == z
			...
			for (int j = 0; j < VERTEX_COUNT; j += 2) {// j == x
				...
				tiles[j / 2][i / 2] = new Tile(x, y, z, j / 2, i / 2, ((j / 2)) % (textureAtlas.getNumTextures() - 1));
			}
		}
	...
	public void constructTileTextureMap() {
		textureIndices = new float[TILE_TEX_INDICE_COUNT * TILE_TEX_INDICE_COUNT];
		// TODO: Check why the fuck there are only (TILECOUNT-2)² Tiles...
		// setting 5
		for (int i = 0; i < textureIndices.length; i++) {
			textureIndices[i] = textureAtlas.getNumTextures() - 1;
		}

		// constructing
		for (int i = 1; i < TILE_COUNT; i++) {
			for (int j = 1; j < TILE_COUNT; j++) {
				textureIndices[(i) * TILE_TEX_INDICE_COUNT + (j)] = tiles[j - 1][i - 1].textureIndex;
			}
		}
	...
	public void makeRandom() {
		for (int i = 0; i < TILE_COUNT + 1; i++) {
			textureIndices[txtindex] = (textureIndices[txtindex] + 1) % textureAtlas.getNumTextures();
	...
	public float getHeight(float x, float z) {
		...
		if (xCoord <= (1 - zCoord)) {
			answer = Maths.barryCentric(new Vector3f(0, heightMap[gridX][gridZ], 0), new Vector3f(1, heightMap[gridX + 1][gridZ], 0), new Vector3f(0,
					heightMap[gridX][gridZ + 1], 1), new Vector2f(xCoord, zCoord));
		} else {
