...
public class Chunk {
    ...
    public boolean dismiss() {
		FileLoader.threadedSaveFile(tiles, new File("saves/" + Game.saveGameName + "/" + x + "x" + z + "/tiles.chk"));
		FileLoader.threadedSaveFile(tileTextureIndices,	new File("saves/" + Game.saveGameName + "/" + x + "x" + z + "/mat.Jmt"));
			tiles = null;
    ...
    private void fetchTiles() {
        tiles = (Tile[][]) FileLoader.loadFile(
                new File("saves/" + Game.saveGameName + "/" + x + "x" + z + "/tiles.chk"));
        tileTextureIndices = (float[]) FileLoader
                .loadFile(new File("saves/" + Game.saveGameName + "/" + x + "x" + z + "/mat.Jmt"));
        ...
        if (tiles == null || tileTextureIndices == null) {
            ...
            for (int i = 0; i < VERTEX_COUNT; i += 2) { // i == z
                for (int j = 0; j < VERTEX_COUNT; j += 2) {// j == x
                    ...
                    //                    tex %= 3;
                    //                }
                    tiles[j / 2][i / 2] = new Tile(x, y, z, j / 2, i / 2, terrain.tex(x[3] + this.x,
                            z[3] + this.z, textureAtlas.getNumTextures()));
                }
            }
        }
    ...
    private int checkSide(Chunk terrain) {
        ...
        if (terrain.getGridZ() < this.getGridZ())
            return 3;
        Logs.terrainRegistering.println("someting went wrong! " + getCoordinateString() + " == "
                + terrain.getCoordinateString() + "??");
        return -1;
    ...
    public void setTileBorder(Tile[] tiles, int index) {
        ...
        case 0:
            ...
            for (int i = 0; i < tiles.length; i++) {
                //works
                tileTextureIndices[(TILE_TEX_INDICE_COUNT - 1) * (TILE_TEX_INDICE_COUNT)
        ...
        case 1:
            ...
            for (int i = 0; i < tiles.length; i++) {
                ...
                tileTextureIndices[(i + 1) * TILE_TEX_INDICE_COUNT + TILE_TEX_INDICE_COUNT
