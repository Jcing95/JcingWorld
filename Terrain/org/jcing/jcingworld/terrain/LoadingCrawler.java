...
public class LoadingCrawler extends Thread {
	...
	private void initTemplate() {
		for (int i = 0; i < Math.ceil((float) Terrain.RENDERDISTANCERADIUS / DataChunk.SIZE) + 1; i++) {
			for (int j = 0; j < Math.ceil((float) Terrain.RENDERDISTANCERADIUS / DataChunk.SIZE) + 1; j++) {
				if (new Vector2f(i, j).length() < Math.ceil((float) Terrain.RENDERDISTANCERADIUS / DataChunk.SIZE) + 1)
	...
	public void run() {
		...
		while (running) {
			...
			for (Point pack : preparePackagesTemplate) {
				terrain.getSaver().preparePackage(pack.x + DataChunk.makePack(terrain.getPlayerChunkPos().x),
						pack.y + DataChunk.makePack(terrain.getPlayerChunkPos().y));
				recent.add(new Point(pack.x + DataChunk.makePack(terrain.getPlayerChunkPos().x),
						pack.y + DataChunk.makePack(terrain.getPlayerChunkPos().y)));
			}
