...
public class Tile implements Externalizable {
    ...
    public void generate(ChunkData data, MapGenerator gen) {
        float SQUARE_SIZE = Chunk.TILE_SIZE / 2;
        //        MapGenerator gen = MainLoop.getGame().getTerrain(data.getTerrainID()).getGenerator();
        int xc = (int) (data.x * Chunk.SIZE);
        int zc = (int) (data.z * Chunk.SIZE);
        float xa[] = { x * SQUARE_SIZE, (x + 1) * SQUARE_SIZE, x * SQUARE_SIZE,
                (x + 1) * SQUARE_SIZE };
        float za[] = { y * SQUARE_SIZE, y * SQUARE_SIZE, (y + 1) * SQUARE_SIZE,
                (y + 1) * SQUARE_SIZE };
...
//        return new Tile(xa, ya, za, gen.tex(xa[3] + xc, za[3] + zc));
    ...
    private Vector3f calcNormal() {
        Vector3f left = Vector3f.sub(new Vector3f(xV[1], yV[1], zV[1]),
                new Vector3f(xV[0], yV[0], zV[0]), null);
        Vector3f right = Vector3f.sub(new Vector3f(xV[0], yV[0], zV[0]),
                new Vector3f(xV[2], yV[2], zV[2]), null);
        normal[0] = Vector3f.cross(left, right, null);

        left = Vector3f.sub(new Vector3f(xV[1], yV[1], zV[1]), new Vector3f(xV[2], yV[2], zV[2]),
                null);
        right = Vector3f.sub(new Vector3f(xV[0], yV[0], zV[0]), new Vector3f(xV[1], yV[1], zV[1]),
                null);
        normal[1] = Vector3f.cross(left, right, null);
        left = Vector3f.sub(new Vector3f(xV[1], yV[1], zV[1]), new Vector3f(xV[2], yV[2], zV[2]),
                null);
        right = Vector3f.sub(new Vector3f(xV[2], yV[2], zV[2]), new Vector3f(xV[3], yV[3], zV[3]),
                null);
        normal[2] = Vector3f.cross(left, right, null);
        left = Vector3f.sub(new Vector3f(xV[1], yV[1], zV[1]), new Vector3f(xV[3], yV[3], zV[3]),
                null);
        right = Vector3f.sub(new Vector3f(xV[2], yV[2], zV[2]), new Vector3f(xV[3], yV[3], zV[3]),
                null);
