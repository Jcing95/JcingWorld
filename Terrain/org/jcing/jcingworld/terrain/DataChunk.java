...
public class DataChunk {
	...
	private Log log = Log.getLog(DataChunk.class);
	...
	// log.info("registered!" + assembledKeys.size() + "(" + xF + "|" + zF + ")");
	// }
	// log.info("put " + x + "|" + z + " to saver ... (" + xF + "|" + zF + ")[" + arr(x) + "]["
	// + arr(z) + "]");
	// }
	...
	public ChunkData get(int x, int z) {
		...
		int zF = makePack(z);
		if (loaded(xF, zF)) {
			log.info("loaded CD (" + x + "|" + z + ") at (" + xF + "|" + zF + ")");
			...
			log.info("UNLOADED CD (" + x + "|" + z + ") at (" + xF + "|" + zF + ")");
			preparePackage(x, z);
			return loaded.get(new Point(xF, zF)).get(x, z);
			// if (load(xF, zF)) {
			// log.info("loading existing CD (" + xF + "|" + zF + ")[" + arr(x) + "|" + arr(z)
			// + "]");
			// if (loaded.get(xF).get(zF)[arr(x)][arr(z)] == null) {
			// ChunkData dta = new ChunkData(x, z, terrain.getTextureAtlas(), terrain);
			// put(x, z, dta);
			...
			// return loaded.get(xF).get(zF)[arr(x)][arr(z)];
			// } else {
			// log.info("tried to load CD from (" + xF + "|" + zF + ")[" + arr(x) + "|" + arr(z)
			// + "] ... creating it");
			// ChunkData dta = new ChunkData(x, z, terrain.getTextureAtlas(), terrain);
			// put(x, z, dta);
			// return dta;
...
//		}
	...
	public void dismiss(Point point) {
		if (loaded.containsKey(point)) {
			...
			log.info("removed " + point);
			loaded.remove(point);
		} else {
			log.info("could not dismiss " + point + " loaded:" + loaded.size());
	...
	public void finish() {
		...
		// log.info("saved " + x + "|" + assembledKeys.get(x));
		// i++;
		// }
		//// frame.dispose();
	...
	public void preparePackage(int x, int z) {
		...
		if (!loaded(xF, zF)) {
			if (!load(xF, zF)) {
				...
				log.info("loaded new Package " + new Point(xF, zF) + " loaded:" + loaded.size());
