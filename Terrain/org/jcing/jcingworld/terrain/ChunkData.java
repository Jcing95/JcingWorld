...
public class ChunkData implements Externalizable {
    ...
    public void constructTileTextureMap() {
        topTileTextureIndices = new float[Chunk.TILE_TEX_INDICE_COUNT
                * Chunk.TILE_TEX_INDICE_COUNT];

        // constructing
        for (int i = 1; i < Chunk.TILE_COUNT; i++) {
            for (int j = 1; j < Chunk.TILE_COUNT; j++) {
                topTileTextureIndices[(Chunk.TILE_TEX_INDICE_COUNT - 1 - j)
                        * Chunk.TILE_TEX_INDICE_COUNT + (i)] = getTile(j - 1, i - 1).textureIndex;
            }
        }
    }
